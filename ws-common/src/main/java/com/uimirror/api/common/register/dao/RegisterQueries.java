...
public class RegisterQueries {
	...
	public static DBObject getQueryForEmailExistanceInTemp(final String email, final int hours){
		return QueryBuilder.start(RegisterQueryConstant.FL_EMAIL).is(email)
				.and(RegisterQueryConstant.FL_CREATED_ON).greaterThanEquals(DateTimeFactory.getStringFromDate(DateTimeFactory.getCurrentDateTime().minusHours(hours)))
				.get();
	}
