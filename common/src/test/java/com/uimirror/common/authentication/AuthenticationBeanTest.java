...
public class AuthenticationBeanTest extends BaseTest{
    ...
    public void testValidForm() {
    	...
    	for(Map<String, Object> authMap : dataSets){
    		...
    		user.put(CommonConstants.PRF_ID, (String)authMap.get("prfid"));
    		user.put(CommonConstants.AUTH_TOKEN, (String)authMap.get("token"));
    		user.put(CommonConstants.AUTH_TOKEN_EXPIRES_ON, DateTimeFactory.getDateAfterSomeDays(1));
    		user.put(CommonConstants.IP, (String)authMap.get("ip"));
    		user.put(CommonConstants.FL_ON, DateTimeFactory.getCurrentDateTimeInString());
    		user.put(CommonConstants.AUTH_TOKEN_INTERVAL, (String)authMap.get("interval"));
    		...
    		Mockito.when(authenticationDao.getNextSessionId()).thenReturn("123");
    		auth = new AuthenticationBean(authMap);
    		Set<ConstraintViolation<AuthenticationBean>> violations = validator.validate(auth);
    		Assert.assertNotNull(violations);
